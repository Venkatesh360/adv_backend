...
class User(Base):
    ...
    Attributes:
        ...
        posts (List[Post]): List of posts made by the user, eagerly loaded.
    
    Relationships:
        posts: Eager-loaded relationship to Post objects with foreign key user_id.
    """
    __tablename__ = "users"
    ...
    # Eager loading: posts are loaded in the same query using a SQL JOIN
    posts = relationship("Post", lazy="joined", backref="user")
...
def show_user_posts():
    ...
    Demonstrates eager loading by fetching users and posts in a single joined query.
    """
    users = session.query(User).all()
    for user in users:
        print(user)
        for post in user.posts:  # Posts already loaded, no additional query triggered
            print("  ", post)
